...
template<> struct vec<2> {
    double x = 0, y = 0;
    vec<2> () {}
    vec<2> (double _x, double _y) : x(_x), y(_y) {}
    double& operator[](const int i)       { assert(i>=0 && i<2); return i ? y : x; }
    double  operator[](const int i) const { assert(i>=0 && i<2); return i ? y : x; }
    double norm2() const { return *this * *this; }
    double norm()  const { return std::sqrt(norm2()); }
    vec<2> normalized() { return (*this)/norm(); }
    vec<2>& Normalized() {*this = (*this) / norm(); return *this;}
};

template<> struct vec<3> {
    double x = 0, y = 0, z = 0;
    vec<3> () {}
    vec<3> (double _x, double _y, double _z) : x(_x), y(_y), z(_z) {}
    double& operator[](const int i)       { assert(i>=0 && i<3); return i ? (1==i ? y : z) : x; }
    double  operator[](const int i) const { assert(i>=0 && i<3); return i ? (1==i ? y : z) : x; }
    double norm2() const { return *this * *this; }
    double norm()  const { return std::sqrt(norm2()); }
    vec<3> normalized() { return (*this)/norm(); }
    vec<3>& Normalized() {*this = (*this) / norm(); return *this;}
};
