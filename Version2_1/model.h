...
public:
    ...
    TGAColor diffuse(const vec2 &uv) { return diffusemap.get(uv.x * diffusemap.width(), uv.y * diffusemap.height()); }
    float specular(const vec2 &uv) { return specularmap.get(uv.x * specularmap.width(), uv.y * specularmap.height())[0]; }
};
